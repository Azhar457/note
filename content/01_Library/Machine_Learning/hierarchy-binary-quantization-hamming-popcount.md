...
## Deepdive — Binary Quantization, Hamming & Popcount dalam Penyerangan ML

### Vektor Serangan

| Teknik | Relevance | Red Team Use |
|--------|-----------|-------------|
| **Hash-based similarity (SimHash, MinHash)** | Near-duplicate detection → evasion | Mimikri konten → bypass similarity |
| **TLSH (Trend Micro Locality Sensitive Hash)** | Malware clustering → signature | Polymorphic code → break TLSH |
| **Binary embedding (1-bit quantization)** | Fast retrieval → model extraction | Steal model via binary representation |
| **Hamming distance** | Fuzzy match → identity clustering | Sockpuppet → blend into cluster |

### Evasion via Hash Collision

```
Malware signature → TLSH hash → cluster match → block
    ↓
Attacker: polymorphic code → minimal change → new TLSH → new cluster → no block
    ↓
Counter: fuzzy threshold + behavior (not only hash) → harder to evade
```

## Referensi
- SimHash — https://www.cs.princeton.edu/courses/archive/spr04/cos598B/...
