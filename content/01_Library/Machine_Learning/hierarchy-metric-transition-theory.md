---
title: — Metric Transition & Game Theory in Security
tags:
- vault
- note
created: 2026-08-14
...
### Security Metrics & Attacker Adaptation

| Metric | Defender View | Attacker Counter |
|--------|---------------|-----------------|
| **MTTD (Detect time)** | Lower = better | Slow & low → avoid trigger |
| **FPR (False Positive)** | Lower = better | Noise injection → drown signal |
| **Coverage (MITRE %)** | Higher = better | Use unmapped TTP |
| **Resolution time** | Lower = better | Multi-vector → confuse analyst |

### Game Theory (Attacker vs Defender)

```
Defender strategy: detect, block, hunt
Attacker strategy: evade, persist, exfil

Equilibrium: No perfect defense → arms race
  ├── Defender: new detection rule
  ├── Attacker: new evasion technique
  └→ Cycle: ATT&CK update → rule update → new bypass
```

## Referensi
- Game Theory (Security) — https://link.springer.com/chapter/10.1007/978-3-319-25133-6_6
