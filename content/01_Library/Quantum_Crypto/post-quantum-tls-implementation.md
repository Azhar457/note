---
title: "Post-Quantum TLS Implementation — Hybrid Key Exchange, ML-KEM, and Handshake Optimization"
tags:
  - post-quantum
  - pqc
  - tls
...
Untuk mencegah ancaman penyadapan data saat ini yang didekripsi di masa depan (_Harvest Now, Decrypt Later_), industri bermigrasi ke standar **Post-Quantum Cryptography (PQC)** yang disetujui NIST pada tahun 2024:
...
ML-DSA menggunakan teknik penolakan penandaan (_Fiat-Shamir dengan Abort_) di atas modul kisi.
...
- **Akibat**: Paket TLS terpaksa dipecah di layer IP. Router di jaringan tengah sering kali membuang paket IP yang difragmentasi (_IP fragmentation drop_) karena alasan kebijakan keamanan firewall, memicu kegagalan handshake TLS (_handshake timeout_).
