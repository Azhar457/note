---
title: "Firmware Reverse Engineering Learning Roadmap — From PCB Dumping to Emulation and Exploitation"
tags:
  - firmware-re
  - hardware-hacking
  - reverse-engineering
...
`binwalk` memindai berkas biner mentah berdasarkan tanda tangan (_signatures_) file terkompresi yang dikenal:
...
_Note_: Untuk emulasi jaringan penuh, gunakan framework **Firmadyne** yang memuat kernel sistem operasi lengkap di atas emulasi QEMU System.
...
- Kredensial statis tersembunyi (_backdoor accounts_ di `/etc/shadow` atau `/etc/passwd`).
...
- Gunakan teknik _hardware debugging_ (JTAG) untuk menghentikan proses boot setelah memori didekripsi, lalu dump isi memori RAM fisik langsung ke komputer eksternal (_RAM dumping_).
