---
title: — Software Engineering & Security Gap
tags:
- vault
- note
created: 2026-08-14
...
## Deepdive — Software Engineering & Security Gap

| Paradigm | Security Challenge | Red Team Exploit |
|----------|-------------------|------------------|
| **Waterfall** | Security = last step → late fix | Pentest before deploy |
| **Agile** | Security debt accumulates → fast ship | Continuous CVE |
| **DevOps** | CI/CD = attack surface | Pipeline poison |
| **DevSecOps** | Shift-left → but tool coverage gap | SAST bypass (custom vuln) |
| **Microservices** | API surface = huge | IDOR, mass assignment |

### SAST/DAST Gap Analysis

```
SAST: source code → pattern match → false positive + miss custom vuln
DAST: runtime → test known vuln → miss custom logic
IAST: agent → real-time → but deployment overhead

Red team: custom vuln (no signature) → bypass SAST/DAST → RCE
```
