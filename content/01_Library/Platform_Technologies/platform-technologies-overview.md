...
| -------------------------- | --------------------- | ------------------------------------------------------------------- | ------------------------------------------------------------------------- | ------------------------------------------------ | ------------------------------------- | ------------------------------ | ----------------------- |
| **io_uring** | OS / Syscall | epoll + aio tidak skalabel, overhead syscall per-operation | Async I/O ring buffer — satu syscall untuk ribuan operasi | PostgreSQL, RocksDB, NGINX, Android | ✅ Production, kernel 5.1+ | [[computer-science-foundations | OS Internals]] |
| **WebAssembly (WASM)** | Runtime | Docker terlalu berat untuk edge/serverless, binary tidak portable | Universal bytecode — satu binary jalan di browser/server/edge/embedded | Cloudflare, Fastly, Docker, Shopify | ✅ Production, WASI maturing | [[cloud-infrastructure         | Cloud]] |
| **CXL** | Hardware / Memory | Server tidak bisa share RAM — AI training butuh RAM massif per node | Memory disaggregation — pool RAM yang bisa di-share antar server via PCIe | Intel, AMD, Samsung, AWS, Google | 🟡 CXL 3.0 spec, early production | [[computer-science-foundations | Computer Architecture]] |
| **Confidential Computing** | Hardware Security | Cloud provider bisa akses data customer di memori | TEE (Trusted Execution Environment) — RAM terenkripsi saat komputasi | Azure, GCP, AWS, IBM | ✅ Production di semua major cloud | [[cryptography-biometrics      | Kriptografi]] |
