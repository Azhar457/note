---
title: AI Systems Index
tags:
- vault
- note
created: 2026-08-14
updated: 2026-08-14
status: pending
cssclasses:
  - wide-table
  - callout
---

# AI Systems

> Sistem AI modern: LLM, RAG, MCP, agentic AI, fine-tuning, evaluasi, dan infrastruktur AI production-grade.

[Kembali ke 01 Library](../_index.md)

> **Total:** 56 files | 0 subfolders

## 📄 Notes
- [[aco-agent-routing-deepdive]] — ACO pheromone untuk memoryless agent dispatch
- [[advanced-ai-algorithms-breakthroughs]]
- [[advanced-chunking-strategies-deepdive]]
- [[adversarial-machine-learning]]
- [[agent-anti-jailbreak-defense-identity]] — defense identity & zero-width injection defense
- [[agent-skills-feedback-sensors-deepdive]]
- [[agentic-ai-mcp-architecture-deepdive]]
- [[agentic-ai-mcp-roadmap]]
- [[ai-comm-protocol-deep-dive]]
- [[ai-engineering-stack-roadmap]]
- [[ai-evaluation-framework]]
- [[ai-governance-ethics]]
- [[ai-governance-ethics2]]
- [[ai-hardware-architecture]]
- [[ai-quality-offline-moc]]
- [[architectural-flaw-detection]]
- [[autonomous-system-design]]
- [[claude-code-plugin-marketplace-deepdive]]
- [[cloud-infrastructure]]
- [[cognitive-architecture-engineering]]
- [[computer-vision-deepdive]]
- [[differential-privacy-praktik]]
- [[distributed-systems]]
- [[docling-deepdive]]
- [[document-parsing-for-rag]]
- [[edge-computing-iot-security-architecture]]
- [[embedded-systems]]
- [[embedding-model-selection-finetuning]]
- [[embodied-ai-robotics]]
- [[embodied-ai-robotics2]]
- [[example-jailbreak]] — raw jailbreak artifact (baseline testing)
- [[gpu-programming-parallel-compute]]
- [[hallucination-mitigation-grounding]]
- [[Note/01_Library/AI_Systems/hierarchy-kernel-bypass-networking]]
- [[Note/01_Library/AI_Systems/hierarchy-llm-ai-systems]] — AI hierarchy stack — layer 0 hardware ke layer 7 product UX, dengan trade-off matriks per layer
- [[hybrid-search-vector-keyword]]
- [[jailbreak-case-study-neko-persona]] — anatomi & dekomposisi jailbreak Neko persona
- [[jailbreak-impact-quantification]] — matematika dampak & probabilitas sukses jailbreak
- [[jailbreak-techniques-taxonomy]] — taksonomi teknik jailbreak & defense
- [[jailbreak-variant-mutation-matrix]] — varian multi-sumber jailbreak
- [[llm-finetuning-toolchain]]
- [[llm-security-red-teaming-attack-surface-ai-layer]]
- [[llm-wiki]]
- [[llmops-ai-infrastructure]]
- [[meta-agent-orchestration]]
- [[multi-agent-orchestration-patterns]]
- [[neurosymbolic-ai]]
- [[neurosymbolic-ai2]]
- [[ollama-vllm-self-hosting-deployment]]
- [[production-model-serving-optimization]]
- [[prompt-engineering-patterns]]
- [[query-transformation-rag]]
- [[rag-data-pipeline-refresh-strategy]]
- [[rag-evaluation-framework]]
- [[rag-pipeline-end-to-end-guide]]
- [[reinforcement-learning-deepdive]]
- [[sandboxed-execution-coding-agents-deepdive]]
- [[simulation-architecture-concepts]]
- [[structured-output-llm-mcp-tool-calling-deepdive]]
- [[system-design]]
- [[test-time-compute-system2]]
- [[vector-database-internals-optimization]]

## 🛠️ Scripts
- [[scripts/zero-width-injection-detector.py]] — deteksi karakter zero-width & homoglyph injection

audited
---
