...
|-------|--------|---------------|-------|--------|
| **Cross-platform implant** | ✅ Windows, Linux, macOS | ❌ Windows only (native) | ✅ Windows (via Demon) | ✅ Windows, Linux, macOS (via Python agent) |
| **Bahasa implant** | Go | C/ASM | C/ASM | PowerShell/Python/C# |
| **GUI** | ❌ (CLI only) | ✅ | ✅ | ✅ (Starkiller) |
| **Multi-user** | ✅ (gRPC) | ✅ (Team Server) | ✅ | ✅ |
| **Sleep obfuscation** | ❌ (opsional via ekstensi) | ✅ (UDRL) | ✅ (Kokkini) | ❌ |
| **Indirect syscalls** | ❌ (opsional via ekstensi) | ✅ (BOF) | ✅ | ❌ |
| **Malleable C2** | ✅ (basic profiles) | ✅ (sangat fleksibel) | ✅ (JSON profiles) | ❌ |
| **WireGuard C2** | ✅ (unik) | ❌ | ❌ | ❌ |
| **Open source** | ✅ (GPLv3) | ❌ (berbayar) | ✅ | ✅ |
...
Sliver menyebut implant sebagai **"sliver"**. Ada beberapa jenis implant:
...
## 📜 Sliver C2 Profiles: Malleable C2 ala Sliver

Sliver mendukung "C2 profiles" untuk menyesuaikan komunikasi implant. Meskipun tidak sefleksibel Malleable C2 Cobalt Strike, profil Sliver cukup untuk meniru traffic umum.
...
## ⚔️ Kill Chain: Operasi Sliver dari Foothold ke Domain Admin
...
- **Binary Analysis**:
  ...
  - YARA rules dapat menargetkan Go runtime signature atau string Sliver spesifik (seperti "sliver", "transport", "beacon").
- **Memory Forensics**:
  - Volatility dapat mendeteksi implant Go dari region memori dengan atribut `PAGE_EXECUTE_READWRITE` yang berisi signature Go.
  - String "sliver" atau "grpc" di memori proses.
- **Sysmon Events**:
...
| Artefak | Deteksi |
|---------|---------|
| **Default User-Agent** | `Go-http-client/2.0` atau `Go-http-client/1.1` — blokir di proxy/WAF. |
...
| **SLIVER_* environment variable** | Beberapa implant menggunakan environment variable dengan prefix `SLIVER_` untuk konfigurasi. |
| **Named Pipe (pivot)** | Nama pipe default mengandung "sliver". Bisa diubah. |
...
## 🛡️ Countermeasures Khusus Sliver
...
- [[cobalt-strike]] — Sliver adalah alternatif open-source yang mendukung cross-platform (Cobalt Strike hanya Windows). Banyak tim merah menggunakan Sliver sebagai secondary C2.
- [[havoc-c2]] — Sama-sama open-source, Havoc lebih unggul di sleep obfuscation dan indirect syscalls, tetapi Sliver unggul di cross-platform dan WireGuard.
- [[empire]] — Empire fokus pada PowerShell; Sliver pada Go binary. Keduanya bisa digunakan paralel untuk target yang berbeda.
- [[metasploit]] — Payload Sliver dapat dikirim melalui modul exploit Metasploit.
- [[bloodhound]] — Sliver dapat menjalankan SharpHound (via execute-assembly) untuk mengumpulkan data AD.
- [[shodan]] — Dapat digunakan untuk mencari server Sliver publik berdasarkan JARM atau port 31337.
