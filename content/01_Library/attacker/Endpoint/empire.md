...
|-----|-------------------|--------|
| 2015–2018 | **Empire** | PowerShell di mana-mana, deteksi minimal, modul lengkap. |
| 2018–2022 | **Cobalt Strike** | Stealth lebih tinggi, malleable C2, BOF, sleep mask. |
| 2022–sekarang | **Havoc / Sliver** | Open-source dengan teknik evasion modern (indirect syscalls). |
...
│  - Modul internal: keylogging, screenshot, mimikatz, etc.    │
└──────────────────────────────────────────────────────────────┘
```
...
| **DNS** | Agent check-in via DNS TXT/MX query ke server, tasking dikirim via DNS response. | Tinggi (bypass firewall, sulit dimonitor) — tetapi lambat. |
...
## 👾 Agent PowerShell: Jantung Empire Klasik
...
3. Loop: tidur (default 5 detik, bisa diatur `Sleep` dan `Jitter`) → GET task dari server → eksekusi task → POST hasil ke server → tidur lagi.

### 3. Eksekusi Task

Task dari server biasanya berupa **PowerShell scriptblock** yang diunduh dan dieksekusi di memori. Karena semuanya adalah PowerShell, tidak ada perbedaan antara "modul" dan "perintah". Semua adalah scriptblock yang bisa diinjeksikan.
...
## 🖥️ Starkiller: GUI Modern untuk Empire
...
|---------|------------------|
| **Registry (WMI Persistence)** | `HKLM\SOFTWARE\Microsoft\Wbem\Scripting` atau `HKLM\SOFTWARE\Microsoft\Wbem\ESS` — objek `__EventFilter` dan `__EventConsumer`. |
...
| **Mimikatz in memory** | String "mimikatz", "gentilkiwi" di memori proses. |
...
- **Event ID 4662**: Operasi replikasi direktori (DCSync) — hanya boleh dari DC.
...
## 🛡️ Countermeasures Khusus Empire
...
| **AMSI** | Jangan hanya mengandalkan AMSI — tapi pastikan AMSI tetap aktif. Monitor upaya menonaktifkan AMSI (memory patching, registry changes). |
...
- [[cobalt-strike]] — Empire dan Cobalt Strike sering digunakan bergantian dalam operasi red team. Empire lebih mudah terdeteksi tetapi memiliki modul PowerShell yang lebih banyak. Cobalt Strike lebih stealth.
- [[havoc-c2]] — Havoc adalah penerus spiritual open-source yang mengatasi kelemahan Empire (sleep obfuscation, indirect syscalls) tetapi kehilangan kedalaman modul PowerShell.
- [[metasploit]] — Payload Empire dapat di-deliver melalui exploit Metasploit. Metasploit juga memiliki modul post-exploitation yang tumpang tindih.
- [[bloodhound]] — Empire memiliki modul `bloodhound3` yang menjalankan SharpHound untuk mengumpulkan data AD, yang kemudian dianalisis di BloodHound.
- **Mimikatz** — Empire mengintegrasikan Mimikatz dalam modul `credentials/mimikatz/*`.
- [[shodan]] — Tidak langsung, tetapi jika Empire C2 server terekspos, bisa ditemukan via Shodan (port 1337, API endpoint).
