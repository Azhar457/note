...
Dokumen Lu:
├── hierarchy-osint-rf.md → OSINT & SIGINT (eksternal intelligence)
├── Application.md → Master Tool Arsenal (semua kategori)
├── endpoint-security (asli) → Ring architecture (threat mapping)
└── THIS DOCUMENT → Blue Team tools open source/freeware
mapped ke setiap Ring
plain
Copy
