...
> [!tip] **Obsidian Vault Scaling & Hygiene** adalah pedoman standardisasi untuk memelihara integritas Obsidian Vault yang berukuran besar ($> 300\text{ files}$). Dokumen ini mendefinisikan aturan penulisan tautan (*wiki links*), standarisasi format frontmatter YAML, manajemen alias untuk mencegah *infinite redirect loop* di Quartz, serta otomatisasi pre-commit audit untuk menjaga kesehatan data vault secara mandiri.
